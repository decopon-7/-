...
右上の**注文票**に書かれた料理に合わせて、半分に切った玉ねぎを仕上げます。
基本（みじん切り）は3つの工程です。
...
- 注文は日を追うごとに増えます。切り方によって工程と細かさ、報酬が変わります。
...
- BGM はメニューを開いている間はこもって聞こえ、開店まで残り20秒を切ると少し速くなります。
...
`tools/make_bgm.py` が、112 BPM・16小節（約34秒）のボサノバ風ループを合成します。
マリンバ風のメロディ、エレピ、ベース、ブラシのドラムの4パートです。
...
`scripts/game_state.gd` の `ORDERS`（注文ごとの切れ目の間隔・みじん切りの目標サイズ）、
