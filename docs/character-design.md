...
**キャラクター生成ツール**: Genspark
本仕様書は最終デザインそのものではなく、Gensparkで画像生成する際のプロンプトのもとになるブリーフ(コンセプト・性格・カラーパレット・造形/モーションルール)。§5に生成プロンプト案を用意しているので、Gensparkに貼り付けて生成し、出力結果を見ながら調整する。

**方向性確認用ラフスケッチ(Artifact)**: https://claude.ai/artifact/B6wsKXRmHXBB8ZHrTqq3LH
(たねちゃん・みどり先生の正面イラストのラフ、カラーパレット、スケール比較を1枚のキャンバスにまとめたもの。あくまで方向性を伝えるための簡易ラフであり、最終デザインはGensparkでの生成結果を正とする)
...
- §5のプロンプト案をGensparkに入力し、たねちゃん・みどり先生の正式なキャラクター原案(正面図・表情差分)を生成する
- 生成結果とラフスケッチ(Artifact)を見比べ、カラーパレット・造形ルールとのズレがあればプロンプトを調整して再生成する
- 各シリーズの台本([scripts/](../scripts/))にある演出メモを、本仕様書のモーションルールと突き合わせて統一する
- モーションガイドライン(まばたき間隔、口パクパターン等)は、Genspark生成結果が確定した後にアニメーション用ガイドとして別途追補する
- 確定した生成結果に合わせて、本仕様書のカラーパレット・造形ルールを最終版に更新する
